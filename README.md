...
`vaultops init` initializes vault server. Besides providing the familiar `vault` command line utility options to connect to the `vault` server, it adds a few more which allow to encrypt the `vault` master keys and root token and store it either locally or in a remote location:
...
**This is not what you should do when initialising your vault servers! This option is only available in the tool for a convenience for local development!**

For real life setups, `vaultops` allows to encrypt the `vault` keys using encryption keys provided by publc cloud providers such as [AWS KMS](https://aws.amazon.com/kms/) or [CGP cloudkms](https://cloud.google.com/kms/). These options are available via `-kms-provider` command line switch. Here is an example how to use GCP cloudkms to encrypt the vault keys when initialising new vault server:
...
**At the moment only AWS KMS and GCP KMS are supported**. I'm planning to add [GPG](https://www.gnupg.org/) shortly.
...
`vaultops` allows you to create a manifest file which can be used when running `vaultops` commands. The manifest is a simple `YAML` (woo, hoo! more `YAML` ᕕ( ᐛ )ᕗ) file which specifies a list of `vault` hosts for initialization and unsealing. In the future it will container arbitrary `vault` API resources to be setup in `vault`.
